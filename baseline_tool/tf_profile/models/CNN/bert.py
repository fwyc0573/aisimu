# get from https://github.com/google-research/bert
# coding=utf-8
...
class BertModelBuilder(object):
  ...
  def create_model(self):
    ...
    input_ids = self.ids_tensor([self.batch_size, self.seq_length],
                                          self.vocab_size)

    input_mask = None
    if self.use_input_mask:
      input_mask = self.ids_tensor(
          [self.batch_size, self.seq_length], vocab_size=2)

    token_type_ids = None
    if self.use_token_type_ids:
      token_type_ids = self.ids_tensor(
    ...
    labels = tf.constant(list(range(num_labels)))
...
def BertFn(bert_config=None, batchsize=None, horovod=False):

  if batchsize is not None:
    bert_config["batch_size"] = batchsize
  bert_model = BertModelBuilder(**bert_config)
  loss, per_example_loss, logits, probabilities = bert_model.create_model()
  learning_rate = 0.123456
  num_train_steps = 1
  num_warmup_steps = 1
  use_tpu = False
  use_multi_gpu = horovod
...
def BertBaseFn(batchsize=None, horovod=False):
  bert_config = {
    "attention_probs_dropout_prob": 0.1,
    "hidden_act": "gelu",
    "hidden_dropout_prob": 0.1,
  ...
  return BertFn(bert_config=bert_config, batchsize=batchsize, horovod=horovod)


def BertLargeFn(batchsize=None, horovod=False):
  bert_config = {
    "attention_probs_dropout_prob": 0.1,
    "hidden_act": "gelu",
    "hidden_dropout_prob": 0.1,
