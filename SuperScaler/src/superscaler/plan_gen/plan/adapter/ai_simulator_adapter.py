...
class AISimulatorAdapter(Adapter):
        ...
        if(not isinstance(node_list, NodeList) and
           not isinstance(node_list, list)):
            ...
            if not self.__check_plan_attr():
                self.__plan = None
                raise RuntimeError("Adapter input attr is illegal")
                return False
    ...
    def __init_communication_attributes(self):
        ...
        for node_raw in self.__plan:
            ...
            if node_raw['op'] == 'Send' or node_raw['op'] == 'Recv' or 'Allreduce' in node_raw['op']:
                node_raw['device_name'] = node_raw['device_name'].replace("GPU", "CPU")
                # print(node_raw['device_name'])
