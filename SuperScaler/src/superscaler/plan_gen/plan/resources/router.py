...
class Router():
    ...
    def get_route_info(self, src_hw_name, dst_hw_name):
        ...
        if self.__hardware_is_computational(src_hw_name) \
            ...
            if (src_hw_name, dst_hw_name) not in self.__routing_log:
                result_list = []
                result_list = self.__bfs_generate_route_info(
                    src_hw_name, dst_hw_name)
    ...
    def __bfs_generate_route_info(self, src_hw_name, dst_hw_name):
        '''Main function of generating routing info, using BFS algorithm,
        return valid paths, return [] if not found
        ...
        Return list is [[Link0, Link1], [Link0, Link2]]
        '''
        hw_queue = [[src_hw_name]]
        link_queue = [[]]
        result_list = []

        min_route_path_length = None

        # Log current hardware
        while hw_queue and link_queue:
            cur_hw = hw_queue[0]
            cur_path = link_queue[0]
            hw_queue.pop(0)
            link_queue.pop(0)

            # print(cur_hw)

            last_hw = self.__hardware_dict[cur_hw[-1]]
            o_links = last_hw.get_outbound_links()

            if min_route_path_length is not None and len(cur_path) >= min_route_path_length:
                break

            for link_dst_name, links in o_links.items():
                # There may be multiple links between src_hw and dst_hw
                if link_dst_name not in cur_hw:
                    if link_dst_name == dst_hw_name:
                        for link in links:
                            new_path = cur_path + [link]
                            if min_route_path_length is None:
                                min_route_path_length = len(new_path)
                            result_list.append(cur_path + [link])
                    elif not self.__hardware_is_computational(link_dst_name):
                        for link in links:
                            link_queue.append(cur_path + [link])
                            hw_queue.append(cur_hw + [link_dst_name])
        # Remove current hardware log
        return result_list
