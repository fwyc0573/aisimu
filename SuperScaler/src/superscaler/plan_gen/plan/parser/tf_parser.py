...
class TFParser(DAGParser):
    ...
    # Available allreduce size on database
    AR_range_list = [
    ...
    def parse_graph(self, graph_path, device="device_0",
                    load_from_memory=False, duplicate_graph=False):

        return self.parse_graphs([graph_path],
                                 [device],
                                 load_from_memory,
                                 duplicate_graph)
    ...
    def parse_graphs(self, graph_paths, devices,
        ...
        if duplicate_graph is True and len(graph_paths) == 0:
            raise ValueError("No graph is found")

        if duplicate_graph is False and len(graph_paths) != len(devices):
        ...
        for node, profiling_data in zip(node_list, profiling_data_list):
            ...
            if self.__Profiler is not None:
                # Linear interpolation for allreduce
                if 'Allreduce' in node['op']:
                    # numElements of gradients for allreduce
                    numElements = 1
                    for shape in profiling_data['output_shapes'][0]:
                        numElements *= shape

                    profiling_data['input_shapes'] = []

                    # Find the lowerbound and upperbound
                    lower_numElements = self.AR_range_list[bisect_left(self.AR_range_list, numElements)-1]
                    higher_numElements = self.AR_range_list[bisect_right(self.AR_range_list, numElements)]
                    
                    # query lowerbound and upperbound from database
                    profiling_data['attr_list'] = [('endpoints', len(devices)),
                                                   ('numElements', lower_numElements)]
                    lower_execution_time =\
                        self.__Profiler.get_node_execution_time(profiling_data)
                    profiling_data['attr_list'] = [('endpoints', len(devices)),
                                                   ('numElements', higher_numElements)]
                    higher_execution_time =\
                        self.__Profiler.get_node_execution_time(profiling_data)

                    # Linear interpolation
                    if lower_execution_time is not None and higher_execution_time is not None and 'avg' in lower_execution_time and 'avg' in higher_execution_time:
                        middle_time = lower_execution_time['avg'] +\
                            (higher_execution_time['avg'] - lower_execution_time['avg']) / (higher_numElements - lower_numElements)
                        execution_time = {'avg': middle_time}
                else:
                    execution_time =\
                        self.__Profiler.get_node_execution_time(profiling_data)
            else:
